module Sunspot
  ...
  class Setup #:nodoc:
    ...
    def initialize(clazz)
      ...
      @more_like_this_factories_cache = Hash.new { |h, k| h[k] = [] }
    ...
    def add_text_field_factory(name, options = {}, &block)
      ...
      if more_like_this
        @more_like_this_factories_cache[field_factory.name] <<  field_factory
      end
    end
    ...
    #
    # Return all text fields configured for more_like_this search
    #
    def more_like_this_fields
      @more_like_this_factories_cache.map do |field_factory|
        field_factory.build
      end
    end
